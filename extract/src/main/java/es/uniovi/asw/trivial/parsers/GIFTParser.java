        ...
        for (int i = 0; i < lineas.length; ++i) {
            ...
            if (linea.toLowerCase().startsWith("$categoria:")
                ...
                if (preguntaActual != null) {
                    System.err.println("No se puede poner categoría dentro "
                            + "de la pregunta (linea " + i + ")");
                    continue;
                }
            ...
            if (linea.contains("{")) {
                if (preguntaActual != null) {
                    System.err.println("Una pregunta termina abruptamente en"
                            + " linea " + i + ", saltando");
                }
            ...
            if (linea.startsWith("~")) {
                ...
                if (numIncorrectas >= Pregunta.NUM_ANSWERS - 1) {
                    System.err.println("La pregunta en linea " + i + " tiene"
                            + " mas respuestas de las que deberia, saltando");
                    preguntaInvalida = true;
                }

                preguntaActual.getWrongAnswers()[numIncorrectas++] = trim(linea
                        .substring(1));
                continue;
            }
