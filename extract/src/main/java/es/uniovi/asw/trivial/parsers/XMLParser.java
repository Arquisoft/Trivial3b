 ...
 * }
 *
 * El orden de las respuestas no es importante. odas las preguntas necesitan una
 * categoria.
 *
...
public class XMLParser implements Parser {
    ...
    public List<Pregunta> parse(String file) {
        ...
        try {
            ...
            expr = xpath.compile("/question/questiondata");

            result = expr.evaluate(doc, XPathConstants.NODESET);

        } catch (Exception e) {
            throw new RuntimeException("Error en la aprertura del fichero");
        }
        ...
        for (int i = 0; i < questionsXML.getLength(); i++) {
            ...
            String questionText = emp.getElementsByTagName("questiontext")
                    .item(0).getTextContent();
            String category = emp.getElementsByTagName("questioncategory")
                    .item(0).getTextContent();
            String idCorrect = emp.getElementsByTagName("correctanswerid")
                    .item(0).getTextContent();
            ...
            for (int j = 0; j < answers.getLength(); j++) {
                ...
                if (ans.getAttribute("id").charAt(0) == idCorrect.charAt(0)) {
                    ...
                    preguntaActual.getWrongAnswers()[numIncorrectas++] = ans
                            .getAttribute("text");
                }
            }
            ...
            if (numIncorrectas != Pregunta.NUM_ANSWERS - 1) {
                System.err.println("La pregunta en linea " + i + " tiene"
                        + " un número incorrecto de respuestas, saltando");
                preguntaInvalida = true;
            }
